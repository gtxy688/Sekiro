...
| 6b | 锁定后 W/S/A/D + Shift | 分别播 `Dodge_Forward` / `Dodge_Back` / `Dodge_Left` / `Dodge_Right`，身体朝 Boss；无方向只按 Shift → `Dodge_Forward` |
...
| P2 | 点「继续」或再按 Esc / 手柄 B（根页） | 菜单关掉，战斗恢复 |
| P2b | 手柄：设置页按 B | 回到暂停根页，不直接关菜单 |
| P2c | 手柄：根页按 Start | 直接关掉暂停 |
| P2d | 手柄上下选菜单 | 当前项亮金底深色字，未选项深灰底浅字 |
| P3 | 点「设置」 | 设置页：音乐 / 音效进度条 + 键位设置 + 返回；右上角叉号点了回到暂停根页 |
| P3c | 看音量行外观 | 百分比不贴着进度条；槽内是连续金色进度条（不是分段方块）；右上角有叉号；按钮仍是暗金 |
| P3b | 点「键位设置」 | 现有改键页，默认「键盘鼠标」Tab，六行战斗键 |
...
| P11 | 设置页拖音乐条 | 旁标 0–100；BGM 立刻变响或变轻 |
| P12 | 音效拉到 0，继续后弹反 | 无音效；BGM 仍按音乐条 |
| P13 | 退出 Play 再进设置页 | 两条滑条仍是上次的值 |
